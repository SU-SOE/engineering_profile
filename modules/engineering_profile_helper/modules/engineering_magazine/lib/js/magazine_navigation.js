(function ($, Drupal) {
  ...
  Drupal.behaviors.engineeringMagazine = {
    attach: function (context, settings) {
      ...
      const topicsPanel = $('.magazine-landing-nav__topics-panel');
      ...
      $(document).on('click', function (e) {
        if (!$(e.target).closest('.topics_item').length &&
          !$(e.target).closest('.magazine-landing-nav__topics-panel').length &&
          isDropdownOpen) {
          closeTopicsDropdown();
        }
      });
      ...
      function setupTopicsKeyboardNavigation() {
        const topicLinks = topicsPanel.find('a');
        ...
        topicLinks.each(function (index) {
          $(this).on('keydown', function (e) {
            switch (e.key) {
              case 'ArrowDown':
                e.preventDefault();
                const nextIndex = (index + 1) % topicLinks.length;
                topicLinks.eq(nextIndex).focus();
                break;

              case 'ArrowUp':
                e.preventDefault();
                const prevIndex = index === 0 ? topicLinks.length - 1 : index - 1;
      ...
      function setTopicLinkActiveStates(currentPath) {
        const topicLinks = topicsPanel.find('.magazine-term a');
        let hasActiveTopicLink = false;
